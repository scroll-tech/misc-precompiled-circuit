...
use crate::utils::{field_to_u32, field_to_u64};
...
pub fn limb_to_u32<F: PrimeField>(limb: &Limb<F>) -> [F; 4] {
    let a = field_to_u32(&limb.value);
    u32_to_limbs(a)
}

pub struct RMD160Chip<F: PrimeField> {
    config: RMD160Config,
...
struct RoundWitness<F: PrimeField> {
    r: u32,      // atomic(b, c, d)
    w0: u32,     // a + r + x + offset
...
impl<F: PrimeField> Chip<F> for RMD160Chip<F> {
...
impl<F: PrimeField> RMD160Chip<F> {
